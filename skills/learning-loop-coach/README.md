# EasyLearning · learning-loop-coach
...
本文件所在目录就是skill目录。GitHub源码位于 `skills/learning-loop-coach/`；复制这个完整文件夹，或解压发行安装包，放入目标环境支持的skills目录。同名已存在时先备份/比较，不覆盖。安装后重新加载技能列表。
